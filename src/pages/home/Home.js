    ...
    const fetchData = async () => {
      try {
        ...
        cities.forEach((city) => {
          ...
          console.log(`Distance to ${city.Name}: ${distance} meters`);
        
          if (distance <= city.Reach && distance < minDistance) {
        ...
        if (closestCity === null) {
          ...
          const pois = await loadPOISByCityId(closestCity.ID);
          setPoisData(pois);
          setError(false);
      ...
      <div className="background">
                ...
                {poisData &&
                  poisData.map((poi) => (
                    <CarouselItem
                      key={poi.ID}
                      poi={poi}
                      onInfoClick={handleToggleInfoPortail}
                    />
                  ))}
              </div>
            </div>
        ...
        {error && (
          <div className="card-background">
            <CardEasteregg
              isConditionCard={isConditionCard}
